(function () {
  ...
  function buildMapAndBind() {
    ...
    if (ul) {
      anchors.forEach(a => {
        ...
        a.addEventListener('click', () => { setSaved(hreflang.toLowerCase()); }, { passive: true });
      });
    } else {
  ...
  function setSaved(hreflang) {
    try { localStorage.setItem(LANG_PREF_KEY, hreflang); } catch (e) { /* ignore */ }
  }
  function removeSaved() {
    try { localStorage.removeItem(LANG_PREF_KEY); } catch (e) { /* ignore */ }
  }
  ...
  function init() {
    ...
    if (sessionStorage.getItem(FIRST_RUN_KEY)) return
    sessionStorage.setItem(FIRST_RUN_KEY, 'true');
